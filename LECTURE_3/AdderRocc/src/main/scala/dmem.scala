...
case object AdderRoccTLB extends Field[Option[TLBConfig]](None)
...
class DmemModule(implicit p: Parameters) extends LazyModule {
  ...
  val node = TLClientNode(Seq(TLMasterPortParameters.v1(Seq(TLMasterParameters.v1("AdderRocc")))))
}
...
class DmemModuleImp(outer: DmemModule)(implicit p: Parameters) extends LazyModuleImp(outer)
  ...
  val tlb = Module(new TLB(false, log2Ceil(coreDataBytes), p(AdderRoccTLB).get)(edge, p))
