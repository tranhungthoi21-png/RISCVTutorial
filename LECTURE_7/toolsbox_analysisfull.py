    187, 83, 194, 226, 16, 130, 61, 188, 86, 52, 117, 76, 227, 70, 74, 237,
    31, 157, 207, 220, 250, 125, 133, 6, 48, 215, 204, 174, 99, 252, 147, 192,
    5, 123, 26, 175, 210, 153, 122, 224, 163, 25, 179, 53, 230, 127, 39, 4,
    177, 162, 223, 176, 182, 244, 178, 46, 132, 241, 211, 120, 140, 27, 35, 44,
    203, 218, 98, 100, 232, 158, 129, 225, 90, 235, 205, 2, 184, 0, 66, 114,
    199, 167, 151, 254, 94, 171, 30, 212, 186, 105, 11, 193, 95, 159, 190, 50,
    13, 8, 217, 229, 45, 216, 249, 10, 170, 92, 89, 65, 19, 33, 87, 247
]
    # Validate SBOX
    if not SBOX:
...
def parity(x: int) -> int:
    """Parity (xor tất cả bit). Hoạt động cho số nguyên bất kỳ độ rộng."""
    return x.bit_count() & 1

def fwht(seq: np.ndarray) -> np.ndarray:
    """
    Fast Walsh–Hadamard Transform in‑place trên mảng ±1,
    kích thước 2^n (n suy ra tự động).
    """
    w = seq.astype(np.int16)             # đủ 16‑bit cho SIZE ≤ 2^8
    h = 1
    while h < len(w):
...
def nonlinearity_bool(f_bits):
    """
    Độ phi tuyến của 1 hàm Boolean n‑biến.
    NL = 2^{n-1} - ½·max|Walsh|.
    """
...
# ------------------------------------------------------------
#  Tính độ phi tuyến S‑box (kích thước bất kỳ 2^n × 2^m)
# ------------------------------------------------------------
def sbox_nonlinearity(sbox):
    SIZE = len(sbox)          # = 2^n
    n = int(np.log2(SIZE))
    if 1 << n != SIZE:
        raise ValueError("Độ dài S‑box phải là 2^n.")
    # Suy ra m từ giá trị lớn nhất
...
print("Độ phi tuyến hàm Boolean     :", nl_bits)
print("Độ phi tuyến hàm nhỏ nhất:", nl_boolean_min)
print("Độ phi tuyến vectơ        :", nl_vector)



def invert_sbox(Sbox):
    inverted_s_box = [0] * len(Sbox)
    for i, value in enumerate(Sbox):
        inverted_s_box[value] = i
    return inverted_s_box
IVSBOX=invert_sbox(SBOX)

print(" Inverse Sbox")
print("SBOX = [")
for i in range(0, len(IVSBOX), 16):
    row = ", ".join(f"0x{val:02X}" for val in IVSBOX[i:i+16])
    print(f"  {row},")
print("]")

nl_bits1, nl_boolean_min1, nl_vector1 = sbox_nonlinearity(IVSBOX)
print("Độ phi tuyến hàm Boolean     :", nl_bits1)
print("Độ phi tuyến hàm nhỏ nhất:", nl_boolean_min1)
print("Độ phi tuyến vectơ        :", nl_vector1)
