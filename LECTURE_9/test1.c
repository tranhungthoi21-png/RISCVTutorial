...
// Thêm hàm keccak_hash_again d? h? tr? d?c ti?p các block m? r?ng c?a SHAKE
...
uint64_t hash[25];
volatile void *keccakCtrl = (void *)0x64004000; // Gi? nguyên d?a ch? co s? nhu b?n g?c c?a b?n
...
int main(int hartid, char **argv) {
    ...
    // --- 4.4. SHAKE128 (4 l?n d?c tuong ?ng 4 hình trên slide) ---
    ...
    keccak_read_done(keccakCtrl); // Báo k?t thúc lu?t d?c 1 tru?c khi hash_again
    printf("\n");

    // SHAKE128 - L?n d?c 2 (Thêm keccak_hash_again)
    printf("Reading the hash value\n");
    hash_len = 21;
    ...
    // SHAKE128 - L?n d?c 3 (Thêm keccak_hash_again)
    printf("Reading the hash value\n");
    hash_len = 21;
    keccak_hash_again(keccakCtrl);
    keccak_read_data(keccakCtrl, hash, hash_len);
    ...
    // SHAKE128 - L?n d?c 4 (Block cu?i cùng -> g?i keccak_hash_again và keccak_read_done)
    printf("Reading the hash value\n");
    hash_len = 1;
    keccak_hash_again(keccakCtrl);
    keccak_read_data(keccakCtrl, hash, hash_len);
    ...
    // --- 4.5. SHAKE256 (4 l?n d?c tuong ?ng 4 hình trên slide) ---
    ...
    // SHAKE256 - L?n d?c 2 (Thêm keccak_hash_again)
    printf("Reading the hash value\n");
    hash_len = 17;
    keccak_hash_again(keccakCtrl);
    keccak_read_data(keccakCtrl, hash, hash_len);
    ...
    // SHAKE256 - L?n d?c 3 (Thêm keccak_hash_again)
    printf("Reading the hash value\n");
    hash_len = 17;
    keccak_hash_again(keccakCtrl);
    keccak_read_data(keccakCtrl, hash, hash_len);
    ...
    // SHAKE256 - L?n d?c 4 (Block cu?i cùng -> g?i keccak_hash_again và keccak_read_done)
    printf("Reading the hash value\n");
    hash_len = 13;
    keccak_hash_again(keccakCtrl);
    keccak_read_data(keccakCtrl, hash, hash_len);
